...
// Adapted from https://raw.githubusercontent.com/hwchase17/langchainjs/main/langchain/src/text_splitter.ts
...
export class TokenTextSplitter
  ...
  constructor(fields: Partial<TokenTextSplitterParams> & { wllama: Wllama }) {
    super(fields);

    this.wllama = fields?.wllama;
  ...
  async splitText(text: string): Promise<string[]> {
    ...
    const input_ids = await this.wllama.tokenize(text);
    ...
    while (start_idx < input_ids.length) {
      const buf = await this.wllama.detokenize(chunk_ids);
      console.log(buf)
      splits.push(this.textDecoder.decode(buf));
